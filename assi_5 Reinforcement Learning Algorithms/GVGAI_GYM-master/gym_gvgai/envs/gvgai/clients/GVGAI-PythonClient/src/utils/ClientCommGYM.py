...
class ClientCommGYM:
    ...
    def __init__(self, game, version, lvl, pathStr):
        self.tempDir = "assignment5log"
        self.addLevel('')   #Level template to be loaded into java

        self.TOKEN_SEP = '#'
        self.io = IOSocket(self.tempDir)
        ...
        if(not os.path.isdir(buildDir)):
            ...
            try:
                self.java = subprocess.Popen(cmd, stdout=subprocess.DEVNULL, cwd=self.tempDir)
    ...
    def step(self,act):
        ...
        if self.sso.isGameOver==True or self.sso.gameWinner=='PLAYER_WINS' or self.sso.phase == "FINISH" or self.sso.phase=="ABORT" or self.sso.phase=="End":
            self.sso.image = misc.imread(os.path.join(self.tempDir, 'gameStateByBytes.png'))
            ...
            if self.sso.phase == "ACT":
                if(self.lastSsoType == LEARNING_SSO_TYPE.IMAGE or self.lastSsoType == "IMAGE" \
                    ...
                    if(self.sso.imageArray):
                        self.sso.convertBytesToPng(self.sso.imageArray, self.tempDir)
                        self.sso.image = misc.imread(os.path.join(self.tempDir, 'gameStateByBytes.png'))
    ...
    def addLevel(self, path):
        lvlName = os.path.join(self.tempDir, 'game_lvl5.txt')
        if(path is ''):
